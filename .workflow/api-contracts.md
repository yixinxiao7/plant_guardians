...
### GROUP — Google OAuth Authentication (T-120)

**Sprint Goal:** Add Google OAuth 2.0 login/register via Passport.js (`passport-google-oauth20`). Two new public endpoints: one initiates the OAuth redirect, one handles the callback. Both are browser-navigation endpoints (not JSON API calls) — they respond with HTTP 302 redirects rather than JSON bodies.

---

#### Token Delivery Mechanism Decision

**Decision:** Query parameters on the frontend redirect URL.

**Rationale:** The frontend SPA runs at a different origin during development (`http://localhost:5173` vs `http://localhost:3000`), which would require `SameSite=None; Secure` for cookies — not viable in local HTTP. Query params are simpler, fully testable without real Google credentials, and already anticipated by SPEC-021 (which explicitly shows `/?token=<jwt>` as an example). The frontend must clean the URL immediately after reading the tokens (per SPEC-021 — `window.history.replaceState`). Access tokens are short-lived (15 min) so exposure in browser history is bounded.

**Agreed delivery shape:**
- Success (new or returning user): `GET /?access_token=<jwt>&refresh_token=<token>`
- Success (account auto-linked): `GET /?access_token=<jwt>&refresh_token=<token>&linked=true`
- Error (user cancelled): `GET /login?error=access_denied`
- Error (all other failures): `GET /login?error=oauth_failed`

Frontend Engineer: acknowledge this decision in `handoff-log.md` before starting T-121.
...
**Description:** Initiates the Google OAuth 2.0 authorization flow. When the browser navigates to this URL, Passport.js redirects the user to Google's consent screen requesting `profile` and `email` scopes. This is a browser navigation endpoint — not an XHR/fetch call. The frontend triggers it via `window.location.href = '/api/v1/auth/google'` or an `<a href>` tag.

**Request:** No body, no query parameters (Passport generates the `state` param internally).

**Response — 302 Found (success):**

Redirects browser to Google's OAuth authorization URL:
```
https://accounts.google.com/o/oauth2/v2/auth?client_id=...&redirect_uri=...&scope=profile+email&state=...&response_type=code
```

This redirect is handled entirely by Passport.js — no JSON response is returned.

**Response — 500 Internal Server Error (degraded):**

If `GOOGLE_CLIENT_ID` or `GOOGLE_CLIENT_SECRET` are not configured, the route must not crash the process. Acceptable behavior: return a redirect to `/login?error=oauth_failed` (302) or a plain 500 JSON response. The route must exist and respond even when real credentials are absent.
...
| HTTP | Code | Scenario |
|------|------|---------|
| 302 → `/login?error=oauth_failed` | — | Google credentials missing/misconfigured; graceful degradation |
| 500 | `OAUTH_NOT_CONFIGURED` | Unexpected failure before redirect (fallback only — prefer the 302 degradation path) |

**Notes:**
- No `Authorization` header is needed — this is a public browser navigation endpoint
- The `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` env vars must be set for this endpoint to perform a real OAuth redirect; without them the route degrades gracefully (no crash, no 500 in production-like configuration)
...
| `state` | string | CSRF state token generated by Passport.js |
| `error` | string | `"access_denied"` if user cancelled on Google's consent screen |
...
|----------|---------------------|
| User cancelled on Google consent screen (`error=access_denied` from Google) | `/login?error=access_denied` |
| Passport exchange failed, DB error, or any unexpected error | `/login?error=oauth_failed` |
...
- `full_name` is populated from Google profile `displayName`; if absent, fall back to concatenating `given_name + ' ' + family_name`; if still absent, use the portion of the email before `@`
- The `google_id` column stores Google's stable unique user ID (the `sub` field from Google's ID token / profile), not the email address

---

### Schema Changes — Sprint 27

#### Migration: Add `google_id` to `users` table

**Migration file:** `backend/migrations/<timestamp>_add_google_id_to_users.js`

**Change:** Add a new nullable, unique column `google_id` to the existing `users` table.
...
Using a partial unique index (`WHERE google_id IS NOT NULL`) so that multiple existing email/password users with `google_id = NULL` do not collide on the unique constraint — only populated `google_id` values must be unique.
...
- All existing `users` rows get `google_id = NULL` — no data migration required
- Existing email/password login flows are entirely unaffected (they do not read or write `google_id`)
- The `password_hash` column remains NOT NULL only for users created via email/password; Google-only users will have `password_hash = NULL` — **this requires confirming the current `password_hash` column constraint in the existing schema**. If `password_hash` is currently `NOT NULL`, the migration must also `ALTER COLUMN password_hash DROP NOT NULL`. See note below.

**⚠️ `password_hash` constraint note:**
The current `users` schema (from Sprint 1) defines `password_hash` as a required string. Google-only users will have no password. The migration must make `password_hash` nullable:

```sql
-- In the same up() migration:
ALTER TABLE users
  ALTER COLUMN password_hash DROP NOT NULL;
```

```sql
-- In down() — restore the NOT NULL constraint:
-- NOTE: Only safe to run rollback if no Google-only users exist (NULL password_hash rows)
ALTER TABLE users
...
|----------|----------|-------------|
| `GOOGLE_CLIENT_ID` | Optional (OAuth disabled if absent) | Google OAuth 2.0 client ID from Google Cloud Console |
| `GOOGLE_CLIENT_SECRET` | Optional (OAuth disabled if absent) | Google OAuth 2.0 client secret |

Both variables added to `.env.example` with placeholder values (`your_google_client_id_here`, `your_google_client_secret_here`). The backend starts cleanly without them — Google OAuth routes degrade gracefully (redirect to `/login?error=oauth_failed`) rather than crashing the process.

**Auto-approval status:** Auto-approved (automated sprint) — Manager reviews in closeout phase.
