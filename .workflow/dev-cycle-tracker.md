...
| T-001 | Implement Login & Sign Up UI | Feature | Frontend Engineer | In Review | P0 | M | 3 | T-D01 | User can create account and log in; form validates; errors shown inline; redirects to inventory on success. **Implemented Sprint 3. 48/48 tests pass.** |
| T-002 | Implement Plant Inventory (Home) screen | Feature | Frontend Engineer | In Review | P0 | L | 3 | T-D02, T-001 | Plants displayed in grid; status badges correct; empty state shown; delete modal works; search filters client-side. **Implemented Sprint 3.** |
| T-003 | Implement Add Plant screen | Feature | Frontend Engineer | In Review | P0 | L | 3 | T-D03, T-001 | Plant created via form; photo upload works; AI advice button triggers modal; success redirects to inventory. **Implemented Sprint 3.** |
| T-004 | Implement Edit Plant screen | Feature | Frontend Engineer | In Review | P1 | M | 3 | T-D04, T-003 | Form pre-populated; changes save correctly; dirty-state detection disables Save until change made. **Implemented Sprint 3.** |
| T-005 | Implement Plant Detail screen | Feature | Frontend Engineer | In Review | P0 | L | 3 | T-D05, T-002 | Care schedule status badges render correctly; "Mark as done" triggers confetti animation and updates state. **Implemented Sprint 3.** |
| T-006 | Implement AI Advice Modal | Feature | Frontend Engineer | In Review | P0 | L | 3 | T-D06, T-003 | Modal opens; accepts photo or text; loading states shown; AI results display; Accept fills form fields. **Implemented Sprint 3.** |
| T-007 | Implement Profile page | Feature | Frontend Engineer | In Review | P1 | M | 3 | T-D07, T-001 | Profile shows user name, email, member since date, plant count, care actions count; logout works. **Implemented Sprint 3.** |
...
| T-021 | Fix LoginPage.test.jsx — 2 failing test selectors | Bug Fix | Frontend Engineer | In Review | P2 | S | 3 | T-001 | **Fixed Sprint 3.** Used `getAllByText` for multiple 'Plant Guardians' matches; used regex `/Email/` for label with required asterisk span. 48/48 tests pass. |
...
| T-023 | Deploy: Re-stage with full frontend implementation | Infrastructure | Deploy Engineer | In Progress | P0 | M | 3 | T-001, T-002, T-003, T-004, T-005, T-006, T-007, T-015, T-016, T-022 | **In Progress (2026-03-24):** Backend running on http://localhost:3000 (bcrypt 6.0.0 — T-022 fix applied, 0 audit vulns). Frontend production build (all 7 screens) serving at http://localhost:4173. 5 DB migrations applied. Seed data present. 48/48 frontend + 40/40 backend tests pass. CORS configured for both :5173 and :4173. Awaiting QA integration test confirmation (T-015, T-016, T-017) to move to In Review. |
