...
| T-031 | Fix profile.test.js intermittent 30s timeout (FB-017) | Bug Fix | Backend Engineer | Backlog | P3 | S | 6 | — | Increase per-test timeout for "should return user profile with stats" (e.g., 60000ms) or optimize PG stats aggregation query. 3 consecutive runs: 44/44, no timeouts. All existing tests still pass. |
| T-032 | Production deployment preparation — nginx reverse proxy + runbook | Infrastructure | Deploy Engineer | Backlog | P2 | M | 6 | — | Create infra/docker-compose.prod.yml with nginx reverse proxy + HTTPS termination. Document production env vars in .env.production.example. Write .workflow/deploy-runbook.md with pre-deploy checklist, startup sequence, rollback steps. Staging unaffected. |
| T-033 | Backend: Implement DELETE /api/v1/auth/account endpoint | Feature | Backend Engineer | Backlog | P2 | M | 6 | — | DELETE /account (authenticated): deletes user + all cascade data (plants, care_schedules, care_actions, photos, refresh tokens). Returns 204. Unit tests: happy path, unauthorized, cascade verify. All 44+ tests pass. API contract added to api-contracts.md. |
| T-034 | Frontend: Implement Delete Account UI on Profile page | Feature | Frontend Engineer | Backlog | P2 | M | 6 | T-033 | Profile page "Delete Account" button wired up. Confirmation modal: "This will permanently delete your account and all your plants. This cannot be undone." On confirm: DELETE /account → clear tokens → redirect to /login. On cancel: modal closes. Unit tests added. All 50+ tests pass. |
