...
| T-008 | Backend: Auth endpoints (register, login, refresh, logout) | Feature | Backend Engineer | Integration Check | P0 | M | 1 | — | Register creates user; login returns JWT; refresh rotates token; logout invalidates refresh token |
| T-009 | Backend: Plants CRUD endpoints | Feature | Backend Engineer | Integration Check | P0 | L | 1 | T-008 | GET /plants, POST /plants, GET /plants/:id, PUT /plants/:id, DELETE /plants/:id all functional with auth |
| T-010 | Backend: Plant photo upload endpoint | Feature | Backend Engineer | Integration Check | P1 | M | 1 | T-009 | POST /plants/:id/photo accepts multipart; stores file; returns URL; validates file type/size. **Fixes applied (2026-03-23):** (1) Added 5 tests to plants.test.js covering happy path (valid JPEG upload), MISSING_FILE error, INVALID_FILE_TYPE error, 401 without auth, and 404 for non-existent plant. (2) Fixed error code from VALIDATION_ERROR to MISSING_FILE when no file is attached. **Code review passed (2026-03-23):** Both fixes verified. Error codes match API contract. 5 tests cover happy + error paths. Security checks pass (auth, ownership, MIME validation, file size limit, UUID filenames). |
| T-011 | Backend: AI advice endpoint (Gemini integration) | Feature | Backend Engineer | Integration Check | P0 | L | 1 | T-009 | POST /ai/advice accepts plant type or image; calls Gemini API; returns structured care recommendations |
| T-012 | Backend: Care actions endpoint (mark as done) | Feature | Backend Engineer | Integration Check | P0 | M | 1 | T-009 | POST /plants/:id/care-actions records action; updates last-done timestamp; returns updated plant state |
| T-013 | Backend: Profile stats endpoint | Feature | Backend Engineer | Integration Check | P1 | S | 1 | T-008, T-009 | GET /profile returns user info + plant count + total care actions completed |
| T-014 | Database schema + migrations | Infrastructure | Backend Engineer | Integration Check | P0 | M | 1 | — | Migrations run up and down cleanly; tables: users, plants, care_schedules, care_actions |
