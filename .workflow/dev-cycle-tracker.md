...
| T-020 | User testing: All 3 MVP flows (novice, AI advice, inventory management) | Spike | User Agent | Backlog | P0 | M | 4 | T-024 | All three user flows from project brief tested in browser; feedback logged to feedback-log.md. **Carried over from Sprint 1, 2 & 3; blocked on T-024 (Monitor health check) returning Deploy Verified: Yes.** |
...
| T-024 | Monitor: Full staging health check with browser verification | Infrastructure | Monitor Agent | Backlog | P0 | S | 4 | — | All 14 API endpoints pass; frontend loads at :5173; auth flow completes in browser; no CORS errors; Deploy Verified: Yes. **Carried over from Sprint 3 — triggered but not completed before sprint close (H-034). Resume in Sprint 4.** |

| T-025 | Configure real Gemini API key + verify AI advice happy path end-to-end | Feature | Backend Engineer | Blocked | P1 | S | 4 | T-024 | Set GEMINI_API_KEY in backend/.env; test POST /ai/advice with photo and plant-type inputs; verify successful JSON response; update qa-build-log.md with happy-path test results. **API contracts written 2026-03-24 (H-037, H-038, H-039). No new endpoints or schema changes. Blocked on T-024 (Monitor health check) before implementation can proceed.** |
| T-026 | Fix AIAdviceModal 502 error state — hide "Try Again", correct message text (FB-004) | Bug Fix | Frontend Engineer | Integration Check | P2 | S | 4 | — | In AIAdviceModal.jsx: for 502/AI_SERVICE_UNAVAILABLE, render only "Close" button (not "Try Again"); update error message to "Our AI service is temporarily offline. You can still add your plant manually." Matches SPEC-006 and H-020 clarification 2. **Implemented 2026-03-24. 50/50 frontend tests pass (2 new tests for this fix).** **CODE REVIEW PASSED (2026-03-24):** Logic correct, spec-compliant, tests cover both 502 and non-502 paths. No security issues. → QA Engineer for integration check (H-042). |
| T-027 | Update SPEC-004 to document redirect-to-detail behavior post-save (FB-005) | Documentation | Design Agent | Backlog | P3 | S | 4 | — | Amend SPEC-004 in ui-spec.md to reflect accepted behavior: after saving Edit Plant, app redirects to /plants/:id (not /). Add rationale note. |
| T-028 | Configure Vite proxy for API routing (technical debt — pre-production) | Infrastructure | Deploy Engineer | Integration Check | P3 | M | 4 | T-024 | **Done (2026-03-24):** Proxy added to `frontend/vite.config.js` (server.proxy + preview.proxy). `api.js` default base changed to `/api/v1` (relative — proxy-friendly). `.env.example` updated to document production-only `VITE_API_BASE_URL`. Build: ✅ 0 errors. Tests: ✅ 50/50 pass. Handoff H-040 → QA Engineer + Monitor Agent for staging re-verification. **CODE REVIEW PASSED (2026-03-24):** Proxy config correct for both dev and preview. Relative URL default in api.js is clean. .env.example well-documented. No secrets, no security issues. → QA Engineer for integration check (H-042). |
