...
| T-022 | npm audit fix — resolve tar transitive vulnerability via bcrypt | Bug Fix | Backend Engineer | In Progress | P3 | S | 3 | — | Run `npm audit fix` in backend; confirm 0 high-severity vulns; re-run `npm test` to confirm 40/40 still pass. **Sprint 3: API contracts phase complete (no new contracts needed). Implementation phase next.** |
