...
| T-028 | Configure Vite proxy for API routing (technical debt — pre-production) | Infrastructure | Deploy Engineer | Backlog | P3 | M | 4 | T-024 | Add Vite proxy config in vite.config.js so frontend calls /api/* are proxied to backend :3000. Eliminates direct cross-origin calls in dev/staging. Required before production deployment. |
