...
| T-025 | Configure real Gemini API key + verify AI advice happy path end-to-end | Feature | Backend Engineer | Backlog | P1 | S | 5 | — | Set GEMINI_API_KEY in backend/.env; test POST /ai/advice with photo and plant-type inputs; verify successful JSON response; update qa-build-log.md with happy-path test results. **T-024 now Done — this task is fully unblocked. API contracts in place (H-037–H-039). No new endpoints or schema changes needed. Sprint 5 contract review complete (api-contracts.md Sprint 5 section). Handoffs H-052 (Frontend) + H-053 (QA) written 2026-03-24. Ready for implementation.** |
...
| T-029 | Fix flaky backend test — intermittent "socket hang up" in POST /plants (FB-010) | Bug Fix | Backend Engineer | Backlog | P3 | S | 5 | — | Investigate and fix intermittent "socket hang up" failure on `POST /api/v1/plants > should create a plant with care schedules`. Consider `--runInBand` in Jest config for CI, or improve supertest/server teardown timing between test suites. All 40/40 backend tests must pass reliably on every run. **No API contract changes. Sprint 5 contract review complete. Handoffs H-052 (Frontend) + H-053 (QA) written 2026-03-24. Ready for implementation.** |
